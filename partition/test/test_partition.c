...
Test(partition, test_handle_dmqp_message_throws_error_on_invalid_args) {
    // arrange
    errno = 0;
    struct dmqp_header header1 = {.method = HEARTBEAT,
                                  .flags = 0,
                                  .queue_entry_timestamp = 0,
                                  .length = 14};
    struct dmqp_message message1 = {.header = header1, .payload = NULL};

    struct dmqp_header header2 = {.method = HEARTBEAT,
                                  .flags = 0,
                                  .queue_entry_timestamp = 0,
    ...
    struct dmqp_header header3 = {.method = HEARTBEAT,
                                  .flags = 0,
                                  .queue_entry_timestamp = 0,
                                  .length = 14};
...
Test(partition,
    ...
    receive_message(mock_sockets[0], &res_header, sizeof(struct dmqp_header));
    ...
    cr_assert_eq(res_header.queue_entry_timestamp, 0);
...
Test(partition, test_handle_dmqp_message_success_on_heartbeat_received) {
    // arrange
    errno = 0;
    struct dmqp_header header = {.method = HEARTBEAT,
                                 .flags = 0,
                                 .queue_entry_timestamp = 0,
                                 .length = 0};
    struct dmqp_message message = {.header = header, .payload = NULL};
...
Test(partition, test_handle_dmqp_message_success_on_push_received) {
    ...
    receive_message(mock_sockets[0], &res_header1, sizeof(struct dmqp_header));
    close(mock_sockets[0]);
    ...
    receive_message(mock_sockets[0], &res_header2, sizeof(struct dmqp_header));
    close(mock_sockets[0]);
    ...
    receive_message(mock_sockets[0], &res_header3, sizeof(struct dmqp_header));
    close(mock_sockets[0]);
...
Test(partition,
    ...
    struct dmqp_header header = {
        .method = POP, .flags = 0, .queue_entry_timestamp = 0, .length = 0};
    struct dmqp_message message = {.header = header, .payload = NULL};
...
Test(partition, test_handle_dmqp_message_success_on_pop_received) {
    ...
    cr_assert_eq(res_header.method, RESPONSE);
    cr_assert_eq(res_header.flags, 0);
    cr_assert_eq(res_header.queue_entry_timestamp, expected_timestamp);
...
Test(partition,
    ...
    struct dmqp_header header = {
        .method = PEEK, .flags = 0, .queue_entry_timestamp = 0, .length = 0};
    struct dmqp_message message = {.header = header, .payload = NULL};
